...
class Embedding:
    ...
    cache: Optional[SafeOptimizedEmbeddingCache]
    _dimension: int  # Dimension of the embeddings

    def __init__(
        self,
        model_name_or_path: str,
        device: str | None = None,
        cache_folder: str | None = None,  # For SentenceTransformer model cache
        trust_remote_code: bool = False,
        enable_cache: bool = True,
        embedding_cache_dir: str | None = None,  # For our embedding cache
    ):
        ...
        self.model = SentenceTransformer(
            model_name_or_path,
            device=device,
            cache_folder=cache_folder,  # Pass the model cache folder here
        ...
        self._dimension = dimension_value

        if self.enable_cache:
            # Ensure embedding_cache_dir is provided if enable_cache is True
            if embedding_cache_dir is None:
                logger.warning(
                    "Embedding cache is enabled but embedding_cache_dir is not set. "
                    "Cache will be disabled or use a default path if SafeOptimizedEmbeddingCache handles it."
                )
                # Defaulting behavior for embedding_cache_dir is handled by SafeOptimizedEmbeddingCache
            self.cache = SafeOptimizedEmbeddingCache(cache_dir=embedding_cache_dir)
            logger.info(
                f"Embedding cache enabled at {self.cache.cache_dir if self.cache else 'N/A'}"
            )
        else:
            self.cache = None
    ...
    def _get_embeddings_from_cache(
        self, texts: List[str], df_for_cache: Optional[Any]
        ...
        Args:
            ...
            df_for_cache: Optional DataFrame for cache key generation.
        ...
        cached_embeddings_list, missing_texts_values = self.cache.get_embeddings_batch(
            texts, self.model_name_or_path, df=df_for_cache
        )

        # The current SafeOptimizedEmbeddingCache.get_embeddings_batch returns:
    ...
    def _encode_and_cache_batch(
        self,
        texts_to_encode_values: List[str],
        batch_size: int,
        df_for_cache: Optional[Any],
    ) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if self.enable_cache and self.cache:
                ...
                success = self.cache.store_embeddings_batch(
                    texts_to_encode_values,
                    embeddings_to_store_list,
    ...
    def embed(
        self, texts: List[str], batch_size: int = 32, df_for_cache: Optional[Any] = None
        ...
        Args:
            ...
            batch_size: Batch size for model encoding.
            df_for_cache: Optional DataFrame associated with the texts, used for cache key generation.
        ...
        final_embeddings, texts_needing_encoding_with_indices = (
            self._get_embeddings_from_cache(texts, df_for_cache)
        )
        ...
        if not texts_needing_encoding_with_indices:
            ...
            # _get_embeddings_from_cache already returns them in order with Nones
            ...
            if len(valid_cached_embeddings) == len(texts):
                ...
                # if _get_embeddings_from_cache is correct.
                logger.error(
        ...
        if texts_to_encode_values:
            newly_encoded_batch_np = self._encode_and_cache_batch(
                texts_to_encode_values, batch_size, df_for_cache
            )
            ...
            else:  # Mismatch in length, should not happen if _encode_and_cache_batch is correct
                logger.error(
        ...
        for i, emb in enumerate(final_embeddings):
            if emb is None:
                ...
                # Option: Fill with zeros of self._dimension, or skip.
                # Current behavior: skip, leading to a potentially shorter result array than input texts.
                # To maintain length, one might do:
                # processed_embeddings.append(np.zeros(self._dimension, dtype=np.float32))
            else:
        ...
        # final_result_array = np.zeros((len(texts), self._dimension), dtype=np.float32)
        # for i, emb in enumerate(final_embeddings):
        #     if emb is not None:
