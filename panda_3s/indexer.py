...
class FaissIndex:
    ...
    Persistence of the index is handled by `save_index` and `load_index` methods.
    Embedding generation and its caching are handled by the `Embedding` class.

    Attributes:
        dimension: The dimensionality of the vectors in the index.
        index: The FAISS index object.
        documents: A list of text documents corresponding to the indexed embeddings.
        document_data: A list of dictionaries containing metadata for each document.
    """
    ...
    def __init__(
        ...
        Args:
            dimension: Vector dimension for the index. Can be set later if the index is built
                       or loaded.
        """
        ...
        logger.info(
            f"FaissIndex initialized. Dimension: {dimension if dimension else 'Not set'}."
        )
    ...
    def _ensure_index_initialized(self, dimension: int) -> None:
        ...
        if self.index is None:
            if self.dimension is None:
                ...
                logger.error(
                    f"Dimension mismatch. Index was configured with {self.dimension}, "
                    f"but new data has dimension {dimension}."
                )
                raise ValueError(
                    f"Dimension mismatch: Index is {self.dimension}D, new data is {dimension}D."
                )
            ...
            logger.info(
                f"Initializing new Faiss index (IndexFlatIP) with dimension={self.dimension}"
            )
            self.index = faiss.IndexFlatIP(self.dimension)
            logger.debug("Faiss index (IndexFlatIP) created.")
        elif self.dimension != dimension:  # Index exists, validate dimension
            logger.error(
    ...
    def add_documents(
        ...
        if self.index is not None:
            ...
            logger.debug(
                f"{len(documents)} documents and their embeddings added. Total documents: {len(self.documents)}."
            )
        else:
    ...
    def add_dataframe(
        ...
        if self.index is not None:
            ...
            logger.debug(
                f"{len(df)} rows processed. Total documents: {len(self.documents)}, Total document_data entries: {len(self.document_data)}."
            )
        else:
    ...
    def search(
        ...
        indices_arr, scores_arr = self._search_faiss(query_embedding, k)
        ...
        if indices_arr.size > 0:
            for idx, score_val in zip(indices_arr[0], scores_arr[0]):
                if 0 <= idx < len(self.documents):
                    ...
                    logger.warning(
                        f"Invalid index {idx} from FAISS search results. Max index: {len(self.documents) - 1}. Skipping."
                    )
    ...
    def save_index(self, index_filepath: str, metadata_filepath: str) -> None:
    ...
    def load_index(self, index_filepath: str, metadata_filepath: str) -> None:
        ...
        if self.index and hasattr(self.index, "d") and self.index.d != 0:
            ...
            if self.index is None and self.dimension is not None:
                logger.info(
                    f"Metadata specifies dimension {self.dimension}, but FAISS index is not yet loaded/trained. Will use this dimension."
                )
            elif (
                self.index
                ...
                logger.info(
                    f"FAISS index is untrained (d=0). Using dimension {self.dimension} from metadata."
                )
        ...
        if self.dimension is not None:
            logger.info(f"Index dimension after load: {self.dimension}")

        # Validate consistency between documents, document_data and index ntotal
        if self.index and self.index.ntotal > 0:
            if not (
                len(self.documents) == self.index.ntotal
                and len(self.document_data) == self.index.ntotal
            ):
                logger.warning(
                    f"Inconsistency after load: FAISS index has {self.index.ntotal} items, "
                    f"but loaded documents count is {len(self.documents)} and "
                    f"document_data count is {len(self.document_data)}. "
    ...
    def clear(self) -> None:
        ...
        self.dimension = None  # Reset dimension
        logger.info(
    ...
    def get_document_count(self) -> int:
        ...
        Returns:
            The number of documents in `self.documents`.
        """
